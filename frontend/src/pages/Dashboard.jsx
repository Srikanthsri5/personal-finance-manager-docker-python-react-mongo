...
export default function Dashboard() {
    ...
    const fetchExpenses = async () => {
        try {
            let url = `http://${window.location.hostname}:8005/api/expenses/`;
            if (viewMode === 'month') {
    ...
    const handleDeleteExpense = async (id) => {
        try {
            const response = await fetch(`http://${window.location.hostname}:8005/api/expenses/${id}`, {
                method: 'DELETE'
            });
            if (response.ok) {
    ...
    const handleExport = (type) => {
        let url = `http://${window.location.hostname}:8005/api/expenses/export/${type}`;
        if (viewMode === 'month') {
            const year = currentDate.getFullYear();
